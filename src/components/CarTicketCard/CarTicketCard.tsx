        ...
        <div className="flex justify-between items-center ">
          <div>
            ...
            <div className="flex gap-4 text-xs text-gray-500 mt-2">
              <span>{selectedCar ? selectedCar.transmission : "Transmission"}</span>
              <span>{selectedCar ? `${selectedCar.seats} Seats` : "Seats"}</span>
              <span>{selectedCar ? selectedCar.fuel_type : "Fuel Type"}</span>
            </div>
          </div>
          <img src={car} alt="Car" className="w-32 h-32 object-contain" />
        </div>
        {showConfirm && (
          <>
            ...
            <div className="w-full container">
              <Button
                onClick={onConfirm}
                className="w-full block text-center bg-[#1E429F] hover:bg-blue-800 text-white font-semibold px-6 py-2 rounded-md shadow-md transition-all justify-center"
              >
                Confirm
              </Button>
            </div>
