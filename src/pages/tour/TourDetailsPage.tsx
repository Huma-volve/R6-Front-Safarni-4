...
import { Star, Heart, ImagePlus, Loader2 } from "lucide-react";
import type { AppDispatch, RootState } from "../../store/store";
import {
  fetchTourDetails,
  fetchTourReviews,
  clearDetails,
} from "../../store/tour/tour";
...
export default function TourDetailsPage() {
  ...
  const slot_id = details?.slots[0].slot_id;
  const max_seats = details?.slots[0].max_seats;

  const BASE_URL = `${import.meta.env.VITE_BASE_URL}`;

  const handleCheckout = async () => {
    if (!slot_id || !max_seats) {
      console.error("Missing slot_id or max_seats");
      return;
    }
    ...
    try {
      ...
      if (res.status === 200 && res.data?.id) {
        console.log("Booking successful:", res.data.id);

        nav(`/checkout?booking_id=${res.data.id}&booking_type=tour`);
      } else {
  ...
  if (detailsError || !details)
    return <div className="p-6 text-rose-600">Failed to load.</div>;

  const price = details.priceFrom.toFixed(2);

  return (
    <div className="min-h-screen bg-[#f6f7fb] text-slate-900 rounded-2xl">
      <div className="p-4 pl-6">
        <BackButton router={-1} />
        <div className="mx-auto w-[min(1000px,95vw)] overflow-hidden rounded-xl border border-slate-200 bg-white">
          <img
            src={details.img}
            alt={details.title}
          ...
          <div className="p-4 md:p-6">
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 text-slate-600 md:grid-cols-3">
              ...
              <div className="text-right md:text-left">
                ...
                {details.location.split(",").slice(-1)}
              </div>
            </div>
            ...
            <div className="mt-5">
              <div className="mb-2 flex items-center justify-between">
                <div className="font-bold">
                  Gallery ({details.slots.length})
                </div>
                <button
                  type="button"
                  className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-sm shadow"
                >
        ...
        <div className="sticky bottom-0 mt-4 flex items-center justify-between gap-3 rounded-lg bg-white/70 p-3 backdrop-blur">
          <div className="text-slate-700">
            Total price:{" "}
            <span className="font-extrabold text-[#2474ff]">${price}</span>
            <span className="text-sm text-slate-500"> /night</span>
          </div>
          <button
            type="button"
            className="h-10 rounded-md bg-[#365cff] px-5 font-bold text-white shadow-md hover:brightness-105"
